import { availableLocales } from '../components/DateTime/components/Locale';
const Message = {
  common: {
    SELECTTION_DUPLICATE_VALUE: 'The selection has duplicate value',
    CHECKED_ITEM_LIST_DUPLICATE_VALUE:
      'The checked item list has duplicate value',
    INVALID_ARGUMENT: 'Error: invalid function arguments'
  },
  control: {
    INVALID_EVENT: 'Invalid event, this function accept only ',
    INVALID_TABLE_FIELDS:
      'Invalid table cell field type, this function accept only '
  },
  radioBtn: {
  ...
  datetime: {
    ...
    INVALID_DATEFORMAT_SEPARATOR:
      'Invalid date format separator. Valid separators are:  "/", "-", " ", ":"'
  },
  colorPicker: {
  ...
  selection: {
    ...
    INVALID_VALUE:
      'Specified invalid options.value not included in options.items[x].value'
  }
};
